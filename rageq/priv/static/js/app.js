...
var Rageq = function () {
  function Rageq() {
    _classCallCheck(this, Rageq);
  }

  _createClass(Rageq, null, [{
    key: "init",
    value: function init() {
      var socket = new _phoenix.Socket('/socket');

      var msg_area = (0, _jquery2.default)("#comment-text-area");

      socket.connect();

      var post_id = parseInt((0, _jquery2.default)("#post").data("id"));
      var user_id = parseInt((0, _jquery2.default)("#user").val());

      var postChannel = socket.channel("post_comments:" + post_id, {});
      postChannel.join().receive("ok", function (resp) {
        console.log("Joined successfully", resp);
      }).receive("error", function (resp) {
        console.log("Unable to join", resp);
      });

      (0, _jquery2.default)(".js-submit-comment").on("click", function (e) {
        var msg = msg_area.val();
        if (msg === "") {
          return;
        }
        postChannel.push("created_comment", { body: msg, post_id: post_id, user_id: user_id });
        msg_area.val("");
      });

      msg_area.off("keypress").on("keypress", function (e) {
        if (e.keyCode == 13) {
          if (msg_area.val() === "") {
            return;
          }
          (0, _jquery2.default)(".js-submit-comment").trigger("click");
        }
      });

      postChannel.on("created_comment", function (payload) {
        (0, _jquery2.default)(".post-post-comment-box").before(createCommentUpdated(payload));
      });

      var createCommentUpdated = function createCommentUpdated(payload) {
